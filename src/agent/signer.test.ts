...
import { createFakeSigner, type SignInput } from "./signer.ts";
...
test("createFakeSigner produces a well-formed signature and commitmentPubkey", async () => {
  ...
  const result = await signer.sign(signInput());
...
test("createFakeSigner is deterministic: same input always yields the same signature (RFC 8032 property, design 4.2)", async () => {
  const signer = createFakeSigner();
  const first = await signer.sign(signInput());
  const second = await signer.sign(signInput());
...
test("createFakeSigner produces a different signature for a different cumulativeAmount", async () => {
  const signer = createFakeSigner();
  const first = await signer.sign(signInput({ cumulativeAmount: "125000" }));
  const second = await signer.sign(signInput({ cumulativeAmount: "150000" }));
...
test("two independently seeded signers never collide", async () => {
  ...
  const inputA = await a.sign(signInput({ cumulativeAmount: "1" }));
  const inputB = await b.sign(signInput({ cumulativeAmount: "1" }));
